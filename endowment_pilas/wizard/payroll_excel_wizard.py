...
class PayrollExcelWizard(models.TransientModel):
    ...
    def action_generate_excel_report(self):
        ...
        if self.plantilla_excel:
            ...
            path = get_module_resource('endowment_pilas', 'data', 'DIVITIASSAS.xlsx')
            if not path or not os.path.exists(path):
                raise UserError(_("Modello DIVITIASSAS.xlsx non trovato nella cartella data del modulo."))
        ...
        for payslip in payslips:
            ...
            for current_leave in relevant_leaves:
                ...
                for col_idx, val in enumerate(data, start=1):
                    ...
                    # TRATAMIENTO DE IDs (Evita notación científica)
                    if col_idx in [2, 3, 97, 98]:
                        cell.data_type = 's'
                        clean_id = str(val or '').upper().replace('.', '').replace('-', '')
                        cell.value = clean_id if clean_id not in ['0', 'FALSE', 'NONE', ''] else None
                    
                    # TRATAMIENTO DE DINERO
                    elif col_idx in [47, 52, 54, 56, 57, 58, 59, 60, 61, 65, 67, 70, 72, 76, 81, 84, 86, 87, 89, 91, 93, 95]:
                        try:
                            num = float(val or 0)
                            cell.value = int(num)
                    ...
                    # TRATAMIENTO DE PORCENTAJES
                    elif col_idx in [53, 66, 77, 85, 88, 90, 92, 94]:
                        try:
                            p = float(val or 0)
                            cell.value = p / 100 if p > 1 else p
